...
def dashboard(request):
	...
	# Sales: total payments in last 30 days
	end = datetime.date.today()
	start = end - datetime.timedelta(days=29)
	sales_qs = Payment.objects.filter(paid_at__date__range=(start, end))
	total_sales = sales_qs.aggregate(total=Sum('amount'))['total'] or 0
	...
	tiles = [
		{
			...
			'created': '15/01/2024',
			'stat_label': 'Last 30d',
			'stat_value': f"{float(total_sales):,.2f}"
		},
		{
			'title': 'Bookings',
...
def sales_history(request):
	"""Show aggregated payments (by amount) over the last 30 days as a chart."""
	end = datetime.date.today()
	start = end - datetime.timedelta(days=29)
	...
	return render(request, 'reports/sales.html', {'labels': labels, 'data': data})
